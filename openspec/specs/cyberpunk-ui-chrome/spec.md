...
Shared cyberpunk visual chrome for non-level UI: menu shell, selection states, footer hints, and match end-state overlays, based on the hybrid designPrototype tokens without arcade-cabinet hardware chrome. The in-match bottom status panel may retain Pang brick chrome separately from this menu/overlay system.

## Requirements

### Requirement: Shared neon UI tokens
Non-level UI SHALL use a consistent cyberpunk token set: dark screen/surface backgrounds, neon cyan primary brand, neon magenta secondary accent, neon gold for selection/focus, and readable idle labels (lighter than pure muted gray when on dark surfaces).
